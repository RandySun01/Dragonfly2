...
func (osc *minioSourceClient) GetContentLength(request *source.Request) (int64, error) {
	...
	fmt.Println("ssssssssssssssssssssssss", objectInfo.Size)
	if err != nil {
...
func (msc *minioSourceClient) IsSupportRange(request *source.Request) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("get minio client host:%s, path:%s %w", request.URL.Host, request.URL.Path, err)
	}
	exists, err := client.BucketExists(request.Context(), request.URL.Host)
	if err != nil {
...
func (msc *minioSourceClient) Download(request *source.Request) (*source.Response, error) {
	...
	client, err := msc.getClient(request.Header)
	if err != nil {
		return nil, fmt.Errorf("get minio client host:%s path:%s err:%w", request.URL.Host, request.URL.Path, err)
	}
	exists, err := client.BucketExists(request.Context(), request.URL.Host)
	if err != nil {
		return nil, fmt.Errorf("get minio bucket %s: %w", request.URL.Host, err)
	...
	fmt.Println("11111request.Header.Get(headers.Range)", request.Header.Get(headers.Range))
	//objectOptions.SetRange()
	...
	fmt.Println("objectInfo.ETag objectInfo.ETag ", objectInfo.ETag)
	return response, nil
}
...
func (msc *minioSourceClient) GetLastModified(request *source.Request) (int64, error) {
	client, err := msc.getClient(request.Header)
	if err != nil {
		return -1, fmt.Errorf("get minio client host:%s path:%s err: %w", request.URL.Host, request.URL.Path, err)
	}
	exists, err := client.BucketExists(request.Context(), request.URL.Host)
	if err != nil {
