...
func (m *Minio) GetObjectMetadata(ctx context.Context, bucketName, objectKey string) (*ObjectMetadata, bool, error) {
	...
	objStat, err := m.client.StatObject(ctx, bucketName, objectKey, minio.StatObjectOptions{})
	if err != nil {
		return nil, false, err
	}
	return &ObjectMetadata{
		Key:                objectKey,
		ContentDisposition: objStat.Metadata.Get(headers.ContentDisposition),
		ContentEncoding:    objStat.Metadata.Get(headers.ContentEncoding),
		ContentLanguage:    objStat.Metadata.Get(headers.ContentLanguage),
